...
public class SymmetricMatrix extends Matrix {
     ...
     * @param a Matrix
     * @exception IllegalArgumentException if the supplied matrix does not have
     * the same dimensions.
     */
    ...
    public static SymmetricMatrix fromComponents(double[][] comp) {
        if (comp.length != comp[0].length) {
            throw new IllegalArgumentException("Non symmetric components: a "
    ...
    /**
     * @return DhbMatrixAlgebra.SymmetricMatrix	product of the tranpose of the
