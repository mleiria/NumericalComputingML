...
public class Bayes {
    ...
    public Map<Integer, Matrix> separateByClass(Matrix dataSet) {
        final Map<Integer, List<Double[]>> separated = new HashMap<>();
        ...
        final Map<Integer, Matrix> separatedMatrix = new HashMap<>();
    ...
    /**
     * For each column in Matrix calculates avg and stdev. The structure is
     * already divided by class
     *
     * @param map
     * @return a Map where the key is the class and the value is a Matrix(2xn)
     * where Matrix(0,i) is the avg of col i and Matrix(1, i) is the stdev of
     * col i
     */
    public Map<Integer, Matrix> getAvgStdev(final Map<Integer, Matrix> map) {

        Map<Integer, Matrix> summaryByClass = new HashMap<>();
        for (final Map.Entry<Integer, Matrix> entry : map.entrySet()) {
            final List<Double[]> avgStdev = new ArrayList<>();
